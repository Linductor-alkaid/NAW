...
namespace {
    ...
    std::string convertToRelativePath(const fs::path& path, const fs::path& projectRoot) {
        try {
            ...
            std::string absRootStr = absRoot.string();
...
static nlohmann::json handleGetProjectStructure(const nlohmann::json& arguments) {
    try {
        ...
        try {
            ...
            for (const auto& entry : dirIter) {
                try {
                    ...
                    // 去重检查
                    if (structurePaths.find(relPathStr) != structurePaths.end()) {
                        continue;
                    }
                    structurePaths.insert(relPathStr);
                    
                    // 根据详细度决定是否包含文件
                    if (detailLevel == "minimal" && fs::is_regular_file(entry)) {
                        // minimal 模式只包含目录
                        continue;
                    }
        ...
        if (jsonStr.size() > maxOutputSize) {
            // 如果输出过大，减少文件列表
            size_t targetSize = maxOutputSize * 0.8; // 保留80%的空间
