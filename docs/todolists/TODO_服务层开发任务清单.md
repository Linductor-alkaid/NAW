...
### 1.1 HTTP客户端封装
- [ ] 选择HTTP库（cpp-httplib / libcurl / 其他）
- [ ] 实现HTTP请求封装
  - [ ] GET/POST请求支持
  - [ ] 请求头管理
  - [ ] 请求体序列化
...
- [ ] 实现配置文件加载
- [ ] 实现配置验证
- [ ] 实现环境变量支持（API Key等敏感信息）
- [ ] 实现配置热重载（可选）

### 1.5 基础数据结构定义
- [ ] 定义任务类型枚举（TaskType）
- [ ] 定义聊天消息结构（ChatMessage）
- [ ] 定义模型配置结构（ModelConfig）
- [ ] 定义请求/响应结构（ChatRequest/ChatResponse）
- [ ] 定义优先级枚举（TaskPriority）
...
- **Phase 1 - 基础设施层**: 0/5 完成
- **Phase 2 - API客户端层**: 0/3 完成
