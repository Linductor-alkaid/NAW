...
#### 1.2.4.2 音频数据预处理
- [x] **音频数据验证**
  - [x] 实现音频数据格式验证（PCM 字节对齐/format 合法性）
  - [x] 实现采样率验证（范围校验）
  - [x] 实现音频长度检查（最小/最大帧数限制）
  - [x] 实现音频质量检查（RMS/峰值/疑似剪裁占比等统计）

- [x] **音频数据标准化**
  - [x] 实现音频数据归一化（peak normalize，目标 -1dBFS）
  - [x] 实现音量调整（gain dB）
  - [x] 实现静音检测和裁剪（头尾静音裁剪 + 最小保留时长）
  - [ ] 实现噪声抑制（可选）
...
- [x] **音频流管理**
  ...
  - [x] 实现音频流错误处理（lastError + 可选 onError 回调；append/capture 失败原因区分）
...
- [x] **音频预处理测试**
  - [x] 音频数据验证测试（纯内存）
  - [x] 音频标准化测试（纯内存：gain/normalize）
  - [x] 静音检测测试（纯内存：trim）

- [ ] **音频流处理测试**
  - [ ] 流式转换测试
  - [ ] 流式预处理测试
  - [x] 流式错误处理测试（append 非对齐/满缓冲等）
...
- [x] **定义验证规则**
  ...
  - [x] 路由配置验证（任务类型有效性等；1.5 TaskType 已就绪，可直接实现）
...
#### 1.4.7.1 配置文件监控
- [x] **实现文件监控**
  - [x] 使用文件系统监控API（如inotify、ReadDirectoryChangesW）（当前采用：轮询 last_write_time）
  - [x] 监控配置文件变化
  - [x] 实现跨平台文件监控（Windows/Linux）
