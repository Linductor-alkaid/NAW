...
- [ ] **优先级队列数据结构设计**
  - [ ] 定义 `RequestItem` 结构体
    ...
    - [ ] `modelId`（选定的模型ID）
    - [ ] `timestamp`（提交时间）
    - [ ] `promise`（std::promise<ChatResponse>，用于异步返回结果）
    - [ ] `cancelToken`（取消令牌，可选）
  - [ ] 定义优先级比较器（`CompareRequestPriority`）
    - [ ] Critical > High > Normal > Low
    - [ ] 同优先级按时间戳排序（FIFO）
  - [ ] 使用 `std::priority_queue` 或自定义优先级队列实现

- [ ] **请求入队/出队**
  - [ ] 实现 `enqueueRequest()` 方法
    - [ ] 生成唯一请求ID（UUID或时间戳+随机数）
    - [ ] 创建 `RequestItem` 对象
    - [ ] 将请求添加到优先级队列
    - [ ] 返回 `std::future<ChatResponse>`（通过 promise）
    - [ ] 线程安全（使用 mutex 保护）
  - [ ] 实现 `dequeueRequest()` 方法
    - [ ] 从优先级队列取出最高优先级请求
    - [ ] 检查请求是否已取消
    - [ ] 返回 `RequestItem` 或 `std::nullopt`（队列为空时）
    - [ ] 线程安全

- [ ] **队列大小限制**
  - [ ] 从 `ConfigManager` 读取 `request_manager.max_queue_size` 配置
  - [ ] 实现队列大小检查（`isQueueFull()`）
  - [ ] 在入队时检查队列大小
    - [ ] 队列满时返回错误或等待（可选：阻塞或立即返回）
    - [ ] 支持配置队列满时的策略（拒绝/等待/丢弃低优先级）
  - [ ] 实现队列统计（当前队列大小、最大队列大小）
...
#### 4.1.3.2 并发控制器实现
- [ ] **按模型限制并发数**
  - [ ] 定义并发控制结构（`ConcurrencyController`）
    - [ ] 按模型ID跟踪当前并发数（`modelId -> currentConcurrency`）
    - [ ] 从 `ModelConfig` 读取 `maxConcurrentRequests` 限制
    - [ ] 使用 `std::atomic` 或 `std::mutex` 保护并发计数
  - [ ] 实现 `checkConcurrencyLimit(const std::string& modelId)` 方法
    - [ ] 获取模型的最大并发数（从 `ModelManager`）
    - [ ] 获取当前并发数
    - [ ] 返回是否可接受新请求（`currentConcurrency < maxConcurrentRequests`）
  - [ ] 实现 `acquireConcurrencySlot(const std::string& modelId)` 方法
    - [ ] 检查并发限制
    - [ ] 递增当前并发数
    - [ ] 返回是否成功获取槽位
  - [ ] 实现 `releaseConcurrencySlot(const std::string& modelId)` 方法
    - [ ] 递减当前并发数
    - [ ] 确保并发数不为负

- [ ] **并发请求计数**
  - [ ] 实现全局并发请求计数（所有模型的总并发数）
  - [ ] 实现按模型的并发请求计数
  - [ ] 实现并发统计查询接口
    - [ ] `getCurrentConcurrency(const std::string& modelId)` - 获取指定模型的当前并发数
    - [ ] `getTotalConcurrency()` - 获取全局总并发数
    - [ ] `getConcurrencyLimit(const std::string& modelId)` - 获取指定模型的并发限制

- [ ] **并发限制检查**
  - [ ] 在请求调度前检查并发限制
  - [ ] 如果达到并发限制，将请求保留在队列中
  - [ ] 实现并发限制等待机制（可选：定期检查或事件通知）
  - [ ] 支持超时（等待时间过长时返回错误）
...
- [ ] **请求分发**
  - [ ] 实现 `dispatchRequest(const RequestItem& item)` 方法
    - [ ] 获取模型配置（从 `ModelManager`）
    - [ ] 检查并发限制（`acquireConcurrencySlot`）
    - [ ] 调用 `APIClient::chatAsync()` 或 `APIClient::chatStream()`
    - [ ] 处理异步结果
      - [ ] 成功：设置 promise 值
      - [ ] 失败：设置 promise 异常或错误响应
    - [ ] 释放并发槽位（`releaseConcurrencySlot`）
    - [ ] 更新统计信息
  - [ ] 支持流式请求分发
    - [ ] 处理流式回调
    - [ ] 聚合流式响应
...
- [ ] **超时管理**
  - [ ] 从 `ConfigManager` 读取 `request_manager.default_timeout_ms` 配置
  - [ ] 从 `ModelConfig` 读取模型特定的超时配置（可选）
  - [ ] 实现请求超时检查
    - [ ] 记录请求开始时间
    - [ ] 定期检查请求是否超时
    - [ ] 超时后取消请求（如果支持）
  - [ ] 实现超时处理
    - [ ] 设置 promise 异常（`std::future_error` 或自定义超时错误）
    - [ ] 释放并发槽位
    - [ ] 更新统计信息（失败计数）
    - [ ] 记录超时日志
...
#### 4.1.3.4 请求取消机制
- [ ] **取消令牌实现**
  - [ ] 定义 `CancelToken` 结构（或使用 `HttpClient::CancelToken`）
    - [ ] `isCancelled()` - 检查是否已取消
    - [ ] `cancel()` - 取消请求
  - [ ] 在 `RequestItem` 中存储 `CancelToken`
  - [ ] 实现请求取消接口（`cancelRequest(const std::string& requestId)`）
    - [ ] 查找请求（在队列中或正在处理中）
    - [ ] 设置取消标志
    - [ ] 如果请求在队列中，从队列移除
    - [ ] 如果请求正在处理，调用 `APIClient` 的取消方法（如果支持）
    - [ ] 设置 promise 异常（取消错误）
    - [ ] 释放并发槽位
...
#### 4.1.3.5 请求统计
- [ ] **统计数据结构**
  - [ ] 定义 `RequestStatistics` 结构体
    - [ ] `totalRequests`（总请求数）
    - [ ] `completedRequests`（完成请求数）
    - [ ] `failedRequests`（失败请求数）
    - [ ] `cancelledRequests`（取消请求数）
    - [ ] `averageResponseTimeMs`（平均响应时间，毫秒）
    - [ ] `minResponseTimeMs`（最小响应时间）
    - [ ] `maxResponseTimeMs`（最大响应时间）
    - [ ] `requestsPerModel`（按模型的请求数统计）
    - [ ] `queueSize`（当前队列大小）
    - [ ] `maxQueueSize`（最大队列大小）
  - [ ] 使用线程安全的数据结构（`std::atomic` 或 `std::mutex`）

- [ ] **统计更新**
  - [ ] 在请求提交时更新统计（`totalRequests++`）
  - [ ] 在请求完成时更新统计（`completedRequests++`，响应时间）
  - [ ] 在请求失败时更新统计（`failedRequests++`）
  - [ ] 在请求取消时更新统计（`cancelledRequests++`）
  - [ ] 按模型统计请求数（`requestsPerModel[modelId]++`）

- [ ] **统计查询接口**
  - [ ] 实现 `getStatistics()` 方法
    - [ ] 返回完整的 `RequestStatistics` 结构
    - [ ] 计算平均响应时间（`totalResponseTime / completedRequests`）
  - [ ] 实现 `getQueueStatistics()` 方法
    - [ ] 返回队列相关统计（当前大小、最大大小、等待时间等）
