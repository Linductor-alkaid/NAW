...
- [ ] **音频数据预处理**
  - [ ] 实现音频格式转换（支持 WAV、PCM、MP3 等）
  - [ ] 实现音频重采样（统一采样率，如 16kHz）
  - [ ] 实现音频归一化（音量标准化）
  - [ ] 实现音频分片（长音频分段处理）
  - [ ] 处理音频格式错误和无效数据

- [ ] **API调用封装**
  - [ ] 实现硅基流动或其他STT API调用接口
  - [ ] 构建STT请求（音频数据、语言参数等）
  - [ ] 处理API认证和错误重试
  - [ ] 实现请求超时控制
...
### 6.6.1 单元测试
- [ ] **SpeechService测试**
  - [ ] STT基础功能测试（音频预处理、API调用、结果解析、置信度处理）
  - [ ] 流式STT测试（音频流处理、增量文本输出）
  - [ ] TTS基础功能测试（文本预处理、音色选择、语速/音调、音频数据返回）
  - [ ] 流式TTS测试（流式音频合成）
  - [ ] VAD测试（音频分析、语音检测算法）
